import { ok, fail, parseJsonBody } from '@/lib/gymcrm/api';
import { canCancelReservation } from '@/lib/gymcrm/domain';
import { PERMISSIONS, hasRole } from '@/lib/gymcrm/permissions';
import { getAuthContext, gymTable, resolveCurrentClientId } from '@/lib/gymcrm/server';
import type { ReservationState } from '@/lib/gymcrm/types';

type Params = { params: Promise<{ id: string }> };

type UpdateReservaBody = {
  estado: ReservationState;
};

const tryPromoteWaitlist = async (
  authCtx: Extract<Awaited<ReturnType<typeof getAuthContext>>, { ok: true }>,
  horarioId: string
) => {
  try {
    await authCtx.client.database.rpc('gymcrm_promover_lista_espera', { p_horario_id: horarioId });
  } catch {
    // Fallback-safe: if RPC is not available yet, reservation cancel still succeeds.
  }
};

export async function PATCH(request: Request, { params }: Params) {
  const authCtx = await getAuthContext();
  if (!authCtx.ok) {
    return authCtx.response;
  }

  const { id } = await params;

  let body: UpdateReservaBody;
  try {
    body = await parseJsonBody<UpdateReservaBody>(request);
  } catch (error) {
    return fail(error instanceof Error ? error.message : 'Body inválido.', 400);
  }

  if (!body.estado) {
    return fail('estado es obligatorio.', 400);
  }

  const reservationResp = await authCtx.client.database
    .from(gymTable('reservas_clases'))
    .select('id, cliente_id, horario_id, estado')
    .eq('gimnasio_id', authCtx.context.gimnasioId)
    .eq('id', id)
    .single();

  if (reservationResp.error || !reservationResp.data) {
    return fail(`Reserva no encontrada: ${reservationResp.error?.message ?? 'unknown error'}`, 404);
  }

  const reservation = reservationResp.data;
  const isStaff = hasRole(authCtx.context.role, PERMISSIONS.reservasStaff);

  if (!isStaff) {
    if (authCtx.context.role !== 'cliente') {
      return fail('No tienes permisos para actualizar reservas.', 403);
    }

    if (body.estado !== 'cancelada') {
      return fail('El cliente solo puede cancelar su reserva.', 403);
    }

    const currentClientId = await resolveCurrentClientId(authCtx, { allowFallback: true, autoCreate: true });
    if (!currentClientId || currentClientId !== reservation.cliente_id) {
      return fail('Solo puedes cancelar tus propias reservas.', 403);
    }

    const horarioResp = await authCtx.client.database
      .from(gymTable('clases_horarios'))
      .select('inicio')
      .eq('gimnasio_id', authCtx.context.gimnasioId)
      .eq('id', reservation.horario_id)
      .single();

    if (horarioResp.error || !horarioResp.data) {
      return fail('No se pudo validar el horario de la reserva.', 400);
    }

    if (!canCancelReservation(horarioResp.data.inicio, new Date(), 30)) {
      return fail('La cancelación debe realizarse con al menos 30 minutos de anticipación.', 400);
    }
  }

  const payload: Record<string, string | null> = {
    estado: body.estado,
  };

  if (body.estado === 'cancelada') {
    payload.cancelada_en = new Date().toISOString();
  }

  const { data, error } = await authCtx.client.database
    .from(gymTable('reservas_clases'))
    .update(payload)
    .eq('gimnasio_id', authCtx.context.gimnasioId)
    .eq('id', id)
    .select('*')
    .single();

  if (error || !data) {
    return fail(`No se pudo actualizar reserva: ${error?.message ?? 'unknown error'}`, 500);
  }

  if (reservation.estado === 'confirmada' && body.estado === 'cancelada') {
    await tryPromoteWaitlist(authCtx, reservation.horario_id);
  }

  return ok(data);
}
